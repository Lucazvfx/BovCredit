"""Rating explicavel para pre-analise de credito pecuario."""
from __future__ import annotations

from services.analysis_engine.experimental_score import calculate_experimental_score


def calcular_rating(*, dscr=None, ltv=None, consistencia=0,
                    confianca='media-baixa', documentos_pendentes=0,
                    comprometimento_pct=None, analysis: dict | None = None) -> dict:
    """Calcula nota 0-100 e faixa A-E.

    E uma politica inicial explicavel. Nao representa PD estatistica ate que
    exista historico real de operacoes pagas e inadimplentes.
    """
    pontos = {}
    try:
    ...
    try:
        ...
        ltv = None
    try:
        consistencia = float(consistencia or 0)
    except (TypeError, ValueError):
        consistencia = 0.0
    try:
        comprometimento_pct = float(comprometimento_pct) if comprometimento_pct is not None else None
    except (TypeError, ValueError):
        comprometimento_pct = None
    ...
    rotulos = {'A': 'baixo risco', 'B': 'risco moderado', 'C': 'risco elevado',
               'D': 'alto risco', 'E': 'nao recomendado'}
    resultado = {
        'score': score,
        'faixa': faixa,
        ...
        'pontos': pontos,
        'observacao': 'Rating indicativo. Deve ser calibrado com historico real de operacoes pagas e inadimplentes.',
    }

    if analysis is not None:
        experimental = calculate_experimental_score(analysis)
        resultado['data_confidence_score'] = (
            analysis.get('data_quality', {}).get('confidence_score')
            if isinstance(analysis, dict) and isinstance(analysis.get('data_quality'), dict)
            else analysis.get('confidence_score') if isinstance(analysis, dict) else None
        )
        resultado['experimental_score'] = experimental['score']
        resultado['experimental_score_details'] = experimental
